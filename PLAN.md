...
> The original attach-only transport evolved into a foreground Pi RPC host controller. The product remains intentionally limited to one configured host and up to five persistent sessions.
...
- Store one host connection (`host`, control port, Plannotator port, token) in Settings. Route up to five isolated RPC runtimes over that connection by stable session ID, with an immutable working directory per runtime and compact tabs in the desktop. Protect the control handshake and health endpoint with an extension-generated, persistent random token: the WebSocket must receive an `auth` message before the server emits a snapshot or accepts commands; use plain HTTP/WebSocket only on the trusted LAN and document that limitation.
...
- Keep the MVP LAN-only and manually configured; avoid cloud accounts, auto-discovery, SSH/process supervision, session-history browsing, file management, voice, non-image attachments, multi-user collaboration, and production-scale hardening.
...
- [x] Add up to five simultaneous persistent Pi runtimes with stable protocol routing, per-session working directories, restart restoration, compact tabs, and one global Plannotator review slot.
...
- Manual end-to-end test from Windows to one remote Pi host: connect, run two working directories simultaneously, switch tabs, stop, reconnect, restore open runtimes, review a plan, review a diff, and resume the agents.
